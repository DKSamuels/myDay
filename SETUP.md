...
| Complex digest decisions | **Sonnet** | If a card requires real judgment — an ambiguous contact, a tricky classification — switch to Sonnet for that decision. |
...
Run this immediately after Step 1, before the email sweep. There are two ways to do it — use either one, or both.
...
1. Query my Gmail for all emails sent or received in the past 90 days.
   ...
   - Sensitive categories: medical providers, financial institutions, legal
     correspondence — exclude silently, do not surface these in review
...
3. From the signature, extract if present:
   - Phone number(s) — label as mobile or office if identifiable
   - LinkedIn URL
...
4. Show me a summary:
   ...
   - New contacts added
   - Commitments extracted — list all of them so I can review
   - Review queue items (if any)
```
---

> **Review the extracted commitments carefully.** Some will be accurate. Some will be false positives — casual mentions that weren't real commitments. Dismiss anything that's not genuine. This trains the extractor.
...
| Standard | Above + LinkedIn | Default — most external meetings |
...
3. Save each brief to briefs/ in my workspace folder.
   Naming: YYYY-MM-DD - [Full Name] - [Company].md
...
> **Briefs folder:** Files land in `briefs/` inside your workspace folder. Open them directly from there. Each file is named `YYYY-MM-DD - [Name] - [Company].md` so they sort cleanly by date.
...
After setup, most of your interaction is reviewing the digest. For everything else, use the `/myday` skill — it wraps all the prompts below into simple commands you can run in Cowork without copy-pasting. See `crm-skill-public.md` for installation.
