...
Open commitments:
- [Any you owe them]
- [Any they owe you, noted separately]
...
4. Confirm. History is preserved; just no new tracking going forward.
...
OVERDUE:
- [COMMITMENT] to [NAME] (was due [DATE])

DUE SOON (within 7 days):
- [COMMITMENT] to [NAME] (due [DATE])

NO DATE SET:
- [COMMITMENT] to [NAME] (from [INTERACTION DATE])
```
...
If this is a new contact with no CRM record, say so and note the source.]
...
relevant to the conversation. Skip if the person is not company-affiliated.]
...
Run the setup guide (`SETUP.md`) before using the skill for the first time.
